<template>
  <section :class="$style.content">
    <div :class="$style.header">
      <i @click="handleback()" class="iconfont">&#xe625;</i>
    ...
    <div :class="$style.box">
      ...
      <input :class="$style.num" type="text" placeholder="请输入用户名" v-model="username">
    </div>
    <div :class="$style.box">
      <label :class="$style.title" for>
        <img src="https://www.baicaif.com/V/home/Wang2/style/images/reg-4.png" alt>
      </label>
      <input :class="$style.num" type="number" placeholder="请输入密码" v-model="password">
    </div>
  ...
  methods: {
    ...
    handleLogin(){
      if(this.username!=""&&this.password!=""){
        if(JSON.parse(localStorage.getItem(this.username))&& JSON.parse(localStorage.getItem(this.username))[0].password == this.password){
